...
// Add a click event handler to close the popup when the close button is clicked
$('.test-drive-popup-close').on("click",function() {
    $('#popupOverlaytestdrive').hide();
    ...
    $(".show-popup-btn").on("click",function(){
        ...
        $(".popup-vehicle-id").val($(this).data("id"))
    ...
    $(".slider-test-drive-form").find(".label-days").on('click', function () {
        if(!$(this).hasClass("disabled")){
            $('.selected_time_s').attr('disabled', false)
            ...
            // $('.test-popup-btn').attr('disabled', false)
...
function getAailability(current_day)
{
    ...
    //     data: {"user_id":100079,"current_day":current_day},
    //     type: 'GET',
    //     success: function(response){
    ...
    $.ajax({
        ...
        headers: {"Authorization": 'Bearer REFx1MRtzW7k3mzY0tJWikz47DjoKtOB'},
        data: {"user_id":100079,"current_day":current_day},
        type: 'GET',
        success: function(response){
            // console.log("****:",response.result.weekDays)
