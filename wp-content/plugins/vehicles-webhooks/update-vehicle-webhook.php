...
function update_vehicle_webhook_endpoint()
{
    register_rest_route('vehicle-webhook/v1', '/update-vehicle', array(
        'methods' => 'PUT',
        'callback' => 'update_vehicle_webhook_callback',
    ));
}

add_action('rest_api_init', 'update_vehicle_webhook_endpoint');

// Callback function for updating vehicle posts
function update_vehicle_webhook_callback($request)
{
    $post_data = $request->get_json_params(); // Get JSON data from the request body
    ...
    $existing_vehicle = get_posts(array(
        'post_type' => 'vehicles',
        'meta_key' => 'id',
        'meta_value' => $id,
        'posts_per_page' => 1,
    ...
    if (empty($existing_vehicle)) {
        $response = array(
            'message' => 'Vehicle with the provided ID not found',
        );

        return rest_ensure_response($response);
    }

    // Get the ID of the existing vehicle post
    $vehicle_id = $existing_vehicle[0]->ID;

    // Update post title or other fields

    wp_update_post(array(
        'ID' => $vehicle_id,
        'post_title' => $post_data["year"] . " " . $post_data["make"] . " " . $post_data["model"] . " " . $post_data['stock'],
        'post_status' => 'publish',
        'post_type' => 'vehicles',
        'meta_input' => array(
            'id' => $post_data['id'],
    ...
    //add_featured_image($vehicle_id, $featured_image_url);
    ...
    $attach_id = wp_insert_attachment($attachment, $upload_file, $vehicle_id);
    update_post_meta($vehicle_id, '_thumbnail_id', $attach_id);
    ...
    if (!empty($gallery_images)) {
        ...
        foreach ($gallery_images as $image_url) {
            ...
            update_post_meta($vehicle_id, 'photo_photos', $attachment_ids);
    ...
    update_post_meta($vehicle_id, 'other_information_transmission', $wp_transmission);
    ...
    update_post_meta($vehicle_id, 'v_condition', $condition);
    $response = array(
