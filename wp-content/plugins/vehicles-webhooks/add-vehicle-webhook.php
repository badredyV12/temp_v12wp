...
// Register the webhook endpoint for adding vehicles
function add_vehicle_webhook_endpoint()
{
    register_rest_route('vehicle-webhook/v1', '/add-vehicle', array(
        'methods' => 'POST',
        'callback' => 'add_vehicle_webhook_callback',
    ));
}

add_action('rest_api_init', 'add_vehicle_webhook_endpoint');

// Callback function for adding vehicle posts
function add_vehicle_webhook_callback($request)
{
    ...
    if (!empty($existing_vehicle)) {
        $response = array(
            'message' => 'Vehicle with the same ID already exists',
            'post_id' => $existing_vehicle[0]->ID,
        );
